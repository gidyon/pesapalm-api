...
func (ctrl *LoanController) ListLoanAccounts(c *gin.Context) {
	...
	if startDate != "" && endDate != "" {
		...
		db = db.Where("loan_account.created_at BETWEEN ? AND ?", startTime, endTime)
	} else if startDate != "" {
		...
		startTime := time.Unix(startDateInt, 0)
		db = db.Where("loan_account.created_at >= ?", startTime)
	} else if endDate != "" {
		...
		endTime := time.Unix(endDateInt, 0)
		db = db.Where("loan_account.created_at <= ?", endTime)
	}
